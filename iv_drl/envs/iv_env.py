...
lives inside the installable ``iv_drl`` package so it can be imported with
    ...
    def step(self, action: np.ndarray):  # type: ignore[override]
        ...
        if self.arb_penalty_lambda > 0:
            try:
                ...
                violation = 0.0
        # Final reward combines prediction error and any arbitrage penalty
        ...
        # Provide next observation or zeros if episode finished
        obs = (
            self._get_obs()
            if not terminated
