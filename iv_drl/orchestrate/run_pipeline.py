...
or ``python -m iv_drl.orchestration.run_pipeline``.
"""
...
_TASKS: list[tuple[str, str, str, Path | None]] = [
    ("iv_drl.features.build_price", "main", "SPX price features", DATA_DIR / "spx_daily_features.parquet"),
    (
        "iv_drl.features.build_iv_surface",
        "main",
        "IV-surface features",
        DATA_DIR / "iv_surface_daily_features.parquet",
    ),
    ("iv_drl.features.fetch_macro", "main", "Macro series", DATA_DIR / "macro_daily_features.parquet"),
    ("iv_drl.panels.merge_features", "merge", "Merge feature sets", DATA_DIR / "spx_iv_drl_state.csv"),
    ("iv_drl.evaluation.evaluate_all", "evaluate_all", "Evaluate forecasts", ARTIFACT_TABLE),
]
